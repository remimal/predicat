...
def predict_using_model(descriptions: List[str], model, k: int = 1):
    ...
    for predicted_labels, predicted_probas in zip(
        all_predicted_labels, all_predicated_probas
    ):
        clean_labels = [
            re.findall("(?<=__label__).*$", str(label))[0] for label in predicted_labels
        ]
...
def preprocess_text(text: str):
    ...
    for (
        ...
        replace_values_ean.items()
    ):  # Substitution des regex appropriés à partir du dictionnaire de règles
